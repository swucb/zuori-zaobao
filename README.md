...
网站由 GitHub Actions 在北京时间每天 07:30 生成静态新闻快照并发布。每日任务会尝试读取原文，并通过智谱 GLM-4.7-Flash 为每条新闻生成一至两段中文摘要；原文或模型暂时不可用时自动使用原文提炼摘要。页面只读取已生成的 `news.json`，不会在访客打开网页时临时抓取新闻，因此无需登录且内容稳定。
